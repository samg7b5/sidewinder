 ...
 * Altered chords
   ...
   * dominant_flat_five
   * dominant_sharp_five
   * lydian_dominant_seventh (dominant sharp eleven)
   * dominant_altered
   * sixth_ninth
   * hendrix_chord
    ...
    if '#' in chord_symbol or 'b' in chord_symbol:
        for n in chord_symbol[1:]:
            if n == '#' or n == 'b':
                ...
                break
    chord_note[0] = chord_note[0].upper()
...
def minor_seventh_flat_five_flat_nine_flat_thirteen(note):
    ...
    """
    return half_diminished_seventh(note) + major_triad(intervals.minor_second(note))
...
def eleventh(note):
    """Build a dominant eleventh chord on note.
...
def major_eleventh(note):
    ...
    ['C', 'E', 'G', 'B', 'F']
    """
    return major_seventh(note) + [intervals.perfect_fourth(note)]
...
def dominant_sharp_five(note):
    """Build the dominant seven sharp 5 (7#5) on note.

    Example:
    >>> dominant_sharp_five('C')
    ['C', 'E', 'G#', 'Bb']
    """
    res = dominant_seventh(note)
    res[2] = notes.augment(res[2])
    return res
...
def dominant_altered(note):
    """Build the altered 7 chord (C7alt = 7 b9 #9 #11 b13) on note.
    
    Example:
    >>> dominant_altered('C')
    ['C', 'E', 'G', 'Bb', 'Db', 'D#', 'F#', 'Ab']
    """
    flat_nine = notes.augment(note)
    sharp_nine = intervals.minor_third(note)
    return dominant_seventh(note) + [flat_nine, sharp_nine, intervals.perfect_fifth(flat_nine), intervals.perfect_fifth(sharp_nine)]
...
def from_shorthand(shorthand_string, slash=None):
    ...
    Elevenths: '11' or 'add11', '7#11', 'm11', 'M11'
...
def determine(chord, shorthand=False, no_inversions=False, no_polychords=False):
    ...
    if chord == []:
        ...
        return determine_seventh(chord, shorthand, no_inversions, no_polychords) # TODO No result for determine(['F', 'G', 'A', 'C']) => add +9 chord
    elif len(chord) == 5:
    ...
    def inversion_exhauster(chord, shorthand, tries, result, polychords):
        ...
        for seventh in sevenths:
            ...
            if seventh == 'M7':
                ...
                if intval4 == 'perfect fourth':
                    add_result('m7b5') # "m7b5add11" https://www.kvraudio.com/forum/viewtopic.php?p=8123543
...
chord_shorthand = {  # Triads Augmented chords Suspended chords Sevenths Sixths
    ...
    'm7b5b9b13': minor_seventh_flat_five_flat_nine_flat_thirteen,
    ...
    '7#5': dominant_sharp_five,
    'dom7alt': dominant_altered,
    '7alt': dominant_altered,
