...
def split_track_at_barnum(track, barnum=1.5): # e.g. halfway through first bar
    ...
    if float(np.floor(barnum)) == float(barnum):
        ...
        for i, notev in enumerate(split_bar):
            ...
            if notev[1]>dur_target-pos and notev[2]==[]:
                print('splitting rest:', notev) 
                restev = Bar()
                restev.place_rest(1/(dur_target-pos))
                restev_rem = Bar()
                restev_rem.place_rest(1/notev[1] - (dur_target-pos))
                ...
                break
        return track.bars[:int(np.floor(barnum)-1)]+[L], [R]+track.bars[int(np.floor(barnum)):]
...
def push_or_pull_next_bar(i, track):
    ...
    while bar_length < 1:
        ...
        rem = 1 - bar_length
        next_bar_shaved, next_bar_rem = split_track_at_barnum(track, i+2 + rem)
        ...
        try:
            bar.place_notes(next_notev[2][0], next_notev[1])
        except IndexError:
            bar.place_notes(None, next_notev[1])
        bar_length += 1/next_notev[1] * next_bar_shaved.meter[0]*(1/next_bar_shaved.meter[1])
        print(bar_length)
#        next_bar = next_bar[1:]
    ...
    while bar_length > 1:
        print('bar too long', i, bar)
        # push forward into the start of the next bar until our length is 1
        rem = bar_length - 1
        back_of_track, excess = split_track_at_barnum(track, i+1 + rem)
        ...
        next_bar =  [last_notev] + [[ev[0]+1/last_notev[1], ev[1], ev[2]] for ev in excess[1]] # insert and shift everything up
...
#        next_bar.notes = [last_notev] + next_bar.notes
        bar_length += (-1)*1/last_notev[1] * bar.meter[0]*(1/bar.meter[1])
        
        track = Track_from_list(back_of_track + [Bar_from_list(next_bar)] + track[i+2:]) # inplace editing while iterating, uh-oh
...
def temporal_realign_bars(track):
    ...
    for i, bar in enumerate(track):
        print(i)
        track = push_or_pull_next_bar(i, track)
        
        
def Bar_from_list(listb=[[0.0, 8.0, None],[0.125, 8.0, ['C-5']]]):
    b = Bar()
    for notev in listb:
        b.place_notes(notev[2], notev[1])
    return b
